...
1. Defining a trading plan with explicit components (rules, risk limits, time windows, setups, conditions)
2. Recording execution details for each trade (entry, exit, size, timing, frequency)
3. Evaluating whether execution met plan requirements
4. Classifying adherence on a per-trade basis (e.g., full adherence, partial adherence, non-adherence)
