...
description: Rules vs Reality Analysis compares a trader's predefined trading rules against their actual executed trades to measure compliance and deviations.
...
1. Defining rules with explicit, measurable criteria (e.g., max daily loss, time windows, position size, trade limits)
2. Recording execution data for each trade (timestamps, size, risk, exits, frequency)
3. Automatically checking each trade against all relevant rules
4. Classifying each rule check as: Pass, Violation, or Skipped (insufficient data)
