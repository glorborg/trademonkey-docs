...
Trade-based filters ("Last 10/20") always look at your most recent closed trades. Day-based filters ("Last 7/30 days") include all trades that closed in that calendar window.

## How to Use Time-Scoped Analytics

1. **Choose a window**
   - In the analytics area (Dashboard, Psychology, Rules), select a time scope such as *Last 20 trades* or *Last 30 days*.

2. **Scan the main metrics**
...
3. **Compare recent vs baseline**
   - If recent win rate, adherence, or conviction are **better** than all-time, behavior is improving.
   - If they are **worse**, something in your recent execution, rules, or psychology has changed.
   - If they are **similar**, behavior is roughly stable.

4. **Focus reviews by window**
...
5. **Drill into details**
   - On the Psychology page: see which emotions, conviction levels, or adherence states dominated in the chosen window.
   - On Rules vs Reality: see which rules you broke most often in that window.
   - On performance views: see whether risk/reward or duration changed compared to your all-time averages.
